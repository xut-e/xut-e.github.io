...
# -----------------
# Paso 4: generar sidebar.html (idéntico)
# -----------------
tree = {}
for n in notes:
...
# -----------------
# Paso 5: generar index.json plano
# -----------------
...
# -----------------
# Paso 6: generar arbol.json jerárquico (tipo Obsidian)
# -----------------
def build_tree(path: Path):
    if path.name == "Z Imagenes":
        return None
    node = {"name": path.name}
    if path.is_dir():
        node["type"] = "directory"
        ...
        for child in sorted(path.iterdir()):
            ...
            if child_node:
                # Para archivos .md añadimos referencia al padre
                if child.is_file():
        ...
        if name.startswith("0."):
            return None
        if re.sub(r"^\d+\.\s*", "", name).lower() == re.sub(r"^\d+\.\s*", "", parent).lower():
            return None
        node["type"] = "file"
...
print("✅ Apuntes generados correctamente con index.json, sidebar.html y arbol.json")
